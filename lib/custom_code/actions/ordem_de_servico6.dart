...
Future<void> ordemDeServico6(
  ...
  final int maxEquipamentosPorPagina = 3; // Definindo como int

  // Dividindo os equipamentos em grupos de no máximo 3
  final gruposDeEquipamentos = <List<EquipamentosOsStruct>>[];
  for (int i = 0; i < equipamentos.length; i += maxEquipamentosPorPagina) {
    gruposDeEquipamentos.add(equipamentos.sublist(
        i,
        i + maxEquipamentosPorPagina > equipamentos.length
            ? equipamentos.length
            : i + maxEquipamentosPorPagina));
  }
  for (var grupo in gruposDeEquipamentos) {
